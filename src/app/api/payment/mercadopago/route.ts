...
export async function POST(request: NextRequest) {
  try {
    ...
    // Preparar itens para a preferencia do Mercado Pago
    // Incluindo TODOS os campos exigidos na homologacao
    const preferenceItems = cartItems.map(
      (item: {
        ...
        id: item.id, // Codigo do item
        title: item.name, // Nome do item
        description: item.description || item.name, // Descricao do item
        quantity: item.quantity, // Quantidade
        unit_price: item.price, // Preco unitario
        currency_id: "BRL",
        picture_url: item.image_url || undefined,
        category_id: item.category || "others", // Categoria do item
      })
    );
    ...
    // Preparar body da preferencia
    const preferenceBody: Parameters<typeof preference.create>[0]["body"] = {
      // Itens do carrinho (com todos os campos)
      ...
      // Descricao na fatura do cartao (statement_descriptor)
      statement_descriptor: "DFLOR ELEGANCE",

      // Metodos de pagamento permitidos
      payment_methods: {
        // Permite PIX, cartao de credito e debito
        excluded_payment_types: [],
        excluded_payment_methods: [],
        installments: 12, // Parcelas maximas
      },
    ...
    // Adicionar notification_url apenas se NAO for localhost
    // (Mercado Pago nao consegue alcancar localhost)
    if (!isLocalhost) {
      preferenceBody.notification_url = `${baseUrl}/api/webhooks/mercadopago`;
    }

    console.log("Criando preferencia MP:", JSON.stringify(preferenceBody, null, 2));

    // Criar preferencia
    let mpPreference;
    try {
      mpPreference = await preference.create({ body: preferenceBody });
      ...
      if (mpError instanceof Error) {
        ...
        errorMessage = String(err.message || err.code || "Erro na API");
    ...
    console.log("Preferencia criada:", mpPreference.id, "para pedido:", orderNumber);

    // Verificar se estamos em modo sandbox (configuravel via env)
    // Como APP_USR- e usado tanto em teste quanto producao, usamos uma variavel
    const isSandbox = process.env.MERCADOPAGO_SANDBOX === "true";

    // Usar URL correta baseado no modo (sandbox ou producao)
    const checkoutUrl = isSandbox
      ? mpPreference.sandbox_init_point
      : mpPreference.init_point;

    console.log(`Modo: ${isSandbox ? "SANDBOX" : "PRODUCAO"}`);
    ...
    return NextResponse.json({
      ...
      productionUrl: mpPreference.init_point,
      isSandbox: isSandbox,
      total: total,
    });
  } catch (error) {
